...
@property (weak, nonatomic) IBOutlet UIView *squareWork;
...
-(void)viewDidLoad {
 ...
 //   [_squareWork addSubview:_square];
    
  //  [_square setAlpha:0];
 //   UITapGestureRecognizer *singleFingerTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleSingleTap:)];
  //  [_squareWork addGestureRecognizer:singleFingerTap];
...
- (void)handleSingleTap:(UITapGestureRecognizer *)recognizer
{
    ...
    printf("tap: %f, %f", point.x ,point.y);
    
    CGFloat x = point.x - _square.frame.size.width/2;
    CGFloat y = point.y - _square.frame.size.height/2;

    [_square setFrame:CGRectMake(x, y, _square.frame.size.width, _square.frame.size.height)];
}
